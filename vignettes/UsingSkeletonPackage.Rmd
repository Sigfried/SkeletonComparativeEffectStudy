...
5. Results, including all diagnostics, will be exported to the prespecified data model. This data only containts aggregated data, and can therefore be shared between study sites. 
...
Please copy the `hiv_test_study` folder. Choose a name for your study package, and change all references from 'hiv_test_study' to your name of choice in the package code. You could use Notepad++'s 'Find in files' option to do this automatically.
...
```
It is also possible to disable positive control sysnthesis altogether by setting the `doPositiveControlSynthesis` variable to `FALSE` in the `R/Main.R` file.
